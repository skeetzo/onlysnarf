...
parser.add_argument('-amount', '--amount', type=valid_amount, 
...
parser.add_argument('-category', '--category', type=str, default='video',
...
parser.add_argument('-categories', '--categories', dest='categories', 
...
parser.add_argument('-cron-user', '--cron-user', type=str, 
  help='the user to run OnlySnarf as', default='root')
##
# -date
# date in MM-DD-YYYY
parser.add_argument('-date', '--date', type=valid_date, default=None,
...
# -debug-delay
# user message delay
parser.add_argument('-debug-delay', '--debug-delay', action='store_true', 
...
##
# download path
parser.add_argument('-download-path', '--download-path', type=str, 
  help='the path to download files to locally', default=DOWNLOAD_PATH)
##
# -duration
# poll or post duration
durationAndExpiration.add_argument('-duration', '--duration', 
  help='the duration', choices=DURATION_ALLOWED, default=None)
##
# -expiration
# date of post or poll expiration
durationAndExpiration.add_argument('-expiration', '--expiration',
  help='the expiration', choices=EXPIRATION_ALLOWED, default=None)
##
# -force-upload
# ignore upload max wait
parser.add_argument('-force-upload', '--force-upload', action='store_true', 
  help='ignore upload max wait attempts')
##
# -download-max
# maximum number of images to download
parser.add_argument('-download-max', '--download-max', type=int, default=IMAGE_DOWNLOAD_LIMIT,
  help='the max number of images to download')
##
# -upload-max
# maximum number of images that can be uploaded
parser.add_argument('-upload-max', '--upload-max', type=int, default=IMAGE_UPLOAD_LIMIT,
  help='the max number of images to upload')
##
# -message-max
# maximum number of images that can be uploaded in a message
parser.add_argument('-message-max', '--message-max', type=int, default=IMAGE_UPLOAD_LIMIT_MESSAGES,
  help='the max number of images to message')
##
# -keywords
# keywords to # in post
parser.add_argument('-keywords', '--keywords', dest='keywords', action='append', default=[], 
  help="the keywords (#[keyword]")
##
# -months
# action: discount
# the number of months to discount for
parser.add_argument('-months', '--months', type=valid_month, default=DISCOUNT_MIN_MONTHS,
...
parser.add_argument('-performers', '--performers', dest='performers', action='append',  default=[],
  help='the performers to list (w/ @[performer]')
# -prefer-local
# prefers local user cache over refreshing first call
parser.add_argument('-prefer-local', '--prefer-local', action='store_true', 
  help='prefer recently cached data')
##
# -price
# the price to be set in a message
parser.add_argument('-price', '--price', type=valid_price, help='the price', default=0)
###
### PATHS ###
# -drive-path
...
parser.add_argument('-users-path', '--users-path', type=str, 
  help='the path to cache users locally', default=USERS_PATH)
###
##
# -questions
# poll questions
parser.add_argument('-questions', '--questions', dest='questions', action='append', default=[],
  help='the questions to ask')
###
# the maximum number of recent users
parser.add_argument('-recent-users-count', '--recent-users-count', default=3,
  type=int, help='the number of users to consider recent')
##
# enables file reduction
parser.add_argument('-enable-reduce', '--enable-reduce', action='store_true', 
  help='enable reducing files under 50 MB')
##
# enables file repair (buggy)
parser.add_argument('-enable-repair', '--enable-repair', action='store_true', 
  help='enable repairing videos as appropriate')
##
# can be set in profile.conf
# root Google drive folder
parser.add_argument('-drive-root', '--drive-root', type=str, default='OnlySnarf',
  help='the Google Drive root folder name')
##
# -save-users
# saves OnlyFans users upon exit
parser.add_argument('-save-users', '--save-users', action='store_true', 
  help='enable saving users locally on exit')
##
# -schedule
# 
parser.add_argument('-schedule', '--schedule', type=valid_schedule, default=None,
  help='the schedule (MM-DD-YYYY:HH:MM)')
##
# -skip-download
parser.add_argument('-skip-download', '--skip-download', action='store_true', 
  help='skip file downloads')
##
# -skip-upload
# skips file upload
parser.add_argument('-skip-upload', '--skip-upload', action='store_true', 
...
# -tags
# @[tag]
parser.add_argument('-tags', '--tags', dest='tags', action='append', default=[],
  help='the tags (@[tag])')
##
# -text
# text for message or upload
parser.add_argument('-text', '--text', type=str, default="",
  help='the text to type')
##
# -time
# time in HH:MM
parser.add_argument('-time', '--time', type=valid_time, default=None,
  help='the time (HH:MM)')
##
# -title
...
##
# -thumbnail
parser.add_argument('-thumbnail', '--thumbnail', action='store_true', 
  help='fix thumbnails when necessary')
##
# -tweet
# enabled tweeting
parser.add_argument('-tweeting', '--tweeting', action='store_true', 
  help='enable tweeting when posting')
##
# -upload-max
# the max number of 10 minute intervals to upload for
parser.add_argument('-upload-max-duration', '--upload-max-duration',  
...
# -users
# the users to target
parser.add_argument('-users', '--users', dest='users', action='append', default=[],
  help='the users to message')
##
# -users-favorite
# list of favorited users
parser.add_argument('-users-favorite', '--users-favorite',  
